...
final_grades = {
		# students t[0] , midterms t[1], finals t[2]
		t[0]:max(t[1], t[2]) 
		for t in # 'dan', max between 80 and 98
...
final_grades = dict(
	zip(
		...
		map(
			...
			zip(midterms, finals) # map the variables 
		)
	)
)
...
avg_grades = dict(
	zip(
		...
		map(
			...
			# student 'dan' : pair[0]+pair[1] = midterms + finals / 2
			# 80 and 98 / 2 
			zip(midterms, finals)
		)
	)
